...
class Controller():
    ...
    def close_gripper(self, env):
        ...
        current_angles['m6'] = -15  #close the gripper
        env.goto_position(current_angles, 1)
    
    def open_gripper(self, env):
        current_angles = env.get_current_angles()
        current_angles['m6'] = 15  # open thr gripper
        env.goto_position(current_angles, 1)
    ...
    def run(self,env, goal_poses):
        ...
        for key, value in goal_poses.items():
            if not key.startswith("b"):
                continue  # Skip intermediate poses or irrelevant keys

            print(f"key: {key}, value: {value}\n")

            target_position, target_orientation = value

            # 1. Move above the block (hovering pose)
            lift_pose = ((target_position[0], target_position[1], 0.2), target_orientation)
            ik_lift = self.inverse_kinematics_fn(lift_pose, env.get_current_angles())
            env.goto_position(ik_lift, 5)
            env.settle(5)

            # 2. Move down to grasp
            ik_down = self.inverse_kinematics_fn(value, env.get_current_angles())
            env.goto_position(ik_down, 5)
            env.settle(5)

            # 3. Close gripper
            self.close_gripper(env)
            env.settle(5)
            ...
            # 5. Move to drop location — for now, same as lift (or define drop_position separately)
            drop_pose = ((target_position[0] + 0.05, target_position[1], target_position[2]), target_orientation)
            drop_lift_pose = ((drop_pose[0][0], drop_pose[0][1], 0.2), drop_pose[1])

            # 5a. Move above drop
            ik_drop_lift = self.inverse_kinematics_fn(drop_lift_pose, env.get_current_angles())
            env.goto_position(ik_drop_lift, 5)
            env.settle(5)

            # 5b. Move down to place
            ik_drop = self.inverse_kinematics_fn(drop_pose, env.get_current_angles())
            env.goto_position(ik_drop, 5)
            env.settle(5)

            # 6. Open gripper
            self.open_gripper(env)
            env.settle(5)
